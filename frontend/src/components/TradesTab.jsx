...
export default function TradesTab({trades,handleCloseTrade,handleClearTrades}){
  ...
  const onClear=async()=>{
    ...
    setClearing(true);await handleClearTrades();setClearing(false);setConfirmClear(false);
  };
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{display:"flex",gap:3}}>
          {[["all","All"],["open","Open"],["win","Wins"],["loss","Losses"]].map(([k,l])=>(
            <button key={k} onClick={()=>setFilter(k)} style={{padding:"5px 12px",borderRadius:5,fontSize:11,cursor:"pointer",border:"1px solid",fontFamily:"var(--font-mono)",background:filter===k?"var(--surface3)":"var(--surface2)",color:filter===k?"var(--text)":"var(--text3)",borderColor:filter===k?"var(--border2)":"var(--border)"}}>{l}</button>
          ))}
        </div>
        ...
        {filtered.length===0?<div className="empty-state"><div className="empty-icon">{trades.length===0?"📋":"🔍"}</div>{trades.length===0?"No trades yet — start the bot":"No trades match the filter"}</div>:(
          <div className="table-wrap">
            <table className="data-table">
              <thead><tr><th>#</th><th>Asset</th><th>Signal</th><th>Entry</th><th>Stop</th><th>TP</th><th>Size</th><th>Conf</th><th>Status</th><th>P&L</th><th>Date</th><th>Time</th><th>Closed</th><th></th></tr></thead>
              <tbody>
                {filtered.map((t,i)=>{
                  ...
                  return(
                    <tr key={i}>
                      ...
                      <td><span style={{fontFamily:"var(--font-display)",fontWeight:700,fontSize:13}}>{t.asset?.replace("/USDT","")}</span><span style={{fontSize:9,color:"var(--text3)"}}>/USDT</span></td>
                      <td><div style={{display:"flex",flexDirection:"column",gap:2}}><span className={`tag tag-${t.signal}`}>{t.signal}</span><span className={`lev-badge ${levCls}`}>{lev}x</span></div></td>
                      ...
                      <td style={{fontFamily:"var(--font-mono)",fontSize:12,fontWeight:600,color:pnlColor}}>{t.pnl!=null?`${t.pnl>=0?"+":""}$${t.pnl.toFixed(4)}`:"—"}</td>
