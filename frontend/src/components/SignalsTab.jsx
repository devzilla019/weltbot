...
export default function SignalsTab({ signals, botStatus, loading }) {
  ...
  const filtered = signals
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "price")
        return (b.signal_data?.market?.price||0) - (a.signal_data?.market?.price||0);
    ...
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:"var(--text2)", marginTop:2 }}>
            {signals.length} assets monitored · SMC v5 with HTF Directional Bias
          </div>
        </div>

        {botStatus?.active_setups?.length > 0 && (
          <div style={{ display:"flex", gap:5, alignItems:"center", flexWrap:"wrap" }}>
            <span style={{ fontSize:10, color:"var(--text3)", fontFamily:"var(--font-mono)" }}>WATCHING:</span>
      ...
      <div className="card" style={{ marginBottom:14, padding:"10px 14px" }}>
        <div style={{ display:"flex", gap:16, alignItems:"center", flexWrap:"wrap" }}>
          ...
          {[
            ["98%+ Confidence","100x","lev-100","BTC/ETH only"],
            ["95%+ Confidence","50x","lev-50","Major pairs"],
            ["90-94%","20x","lev-20","All assets"],
            ["85-89%","10x","lev-10","All assets"],
          ].map(([label,lev,cls,note]) => (
            <div key={lev} style={{ display:"flex", alignItems:"center", gap:5 }}>
              <span className={`lev-badge ${cls}`}>{lev}</span>
              <span style={{ fontSize:10, color:"var(--text3)" }}>{label}</span>
              <span style={{ fontSize:9, color:"var(--text3)", opacity:0.6 }}>({note})</span>
            </div>
          ))}
        </div>
      </div>
              ...
              {[["all","All"],["active","Active"],["buy","Buy"],["sell","Sell"],["hold","Hold"]].map(([k,l]) => (
                <button key={k}
                  ...
                  style={{
                    ...
                    borderColor: filter===k ? "var(--info)" : "var(--border)",
                    fontFamily:"var(--font-mono)",
                  }}
                >
                  {l} <span style={{ opacity:0.7, fontSize:10 }}>{counts[k]}</span>
                </button>
              ))}
            </div>
            <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:6 }}>
              <span style={{ fontSize:10, color:"var(--text3)" }}>Sort:</span>
              {[["confidence","Confidence"],["price","Price"],["symbol","Symbol"]].map(([k,l]) => (
                <button key={k}
                  onClick={() => setSortBy(k)}
                  style={{
